...
# 正式部署主機為 ARM64；部署到 x86_64 主機時可覆寫為 linux/amd64。
...
# Exporting the value makes it override stale RUNTIME_IMAGE_TAG entries in a
# deployment's compose/.env while keeping unchanged images reusable.
RUNTIME_IMAGE_FINGERPRINT := $(shell { git hash-object .docker/Dockerfile; printf '%s\n' '$(PHP_IMAGE)' '$(COMPOSER_IMAGE)' '$(RUNTIME_PLATFORMS)'; } | git hash-object --stdin | cut -c1-12)
ifeq ($(strip $(RUNTIME_IMAGE_FINGERPRINT)),)
$(error Unable to calculate the runtime image fingerprint)
endif
RUNTIME_IMAGE_TAG := php$(PHP_VERSION)-$(RUNTIME_IMAGE_FINGERPRINT)
...
runtime-ready: runtime-publish-if-needed
	@if docker image inspect "$(RUNTIME_IMAGE)" >/dev/null 2>&1; then \
		if docker run --rm --entrypoint sh "$(RUNTIME_IMAGE)" -c \
			...
			echo "Reuse verified runtime image: $(RUNTIME_IMAGE)"; \
		else \
			echo "Runtime image is missing Imagick or the Traditional Chinese font; rebuilding: $(RUNTIME_IMAGE)"; \
			( $(COMPOSE_CMD) build $(APP_SERVICE) ); \
		fi; \
	elif ( $(COMPOSE_CMD) pull $(APP_SERVICE) ); then \
		if docker run --rm --entrypoint sh "$(RUNTIME_IMAGE)" -c \
			'php -r '\''exit(extension_loaded("imagick") ? 0 : 1);'\'' && test -r /usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc'; then \
			echo "Pulled and verified runtime image: $(RUNTIME_IMAGE)"; \
		else \
			echo "Pulled runtime image is missing Imagick or the Traditional Chinese font; rebuilding: $(RUNTIME_IMAGE)"; \
			( $(COMPOSE_CMD) build $(APP_SERVICE) ); \
		fi; \
	else \
