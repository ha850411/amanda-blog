...
所有項目都會顯示**兩隊各自近 5 場的勝敗**（W 勝／L 敗／D 和，由新到舊），電競以已完賽的系列賽計算。未滿 5 場會標示實際場數，資料失敗顯示「暫無資料」；查過去日期時，以該場開賽前的結果計算，未來賽程則以查詢當下為準。MLB 也會查詢對手的近況，不受追蹤球隊清單限制；先批次查近 30 天，僅在不足 5 場時向前延伸至 400 天。新版賽程圖將隊名、即時比分、賠率、近況色塊與雙方交手分區呈現，文字降級回覆也包含近況。

效能採用**批次請求、最多 5 個並行請求、同次查詢去重及先篩選後補資料**，不讀寫賽程、戰績或盤口快取。bo3.gg 近況與交手共用同一批請求，已取得的對局詳情會提供給後續查詢使用；MLB 則合併可見隊伍與對手的查詢，並限制 API 回傳欄位。每次指令仍重新向來源取得資料。單一賽程來源失敗時會保留另一來源的結果並顯示提示。
